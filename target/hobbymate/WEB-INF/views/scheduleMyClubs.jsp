		...
		<div style="width:100%">
			<div id="main-content" class="flex-grow-1 p-4">
				...
				<div class="d-flex align-items-center gap-3 mb-4">
					...
					<h3 class="mb-0 text-secondary">전체 모임 일정</h3>
	
					<form class="d-flex"
						action="${pageContext.request.contextPath}/schedule/myclubs"
						...
						<div class="calendar-card shadow-sm">
							...
							<div
								...
								<button class="btn btn-sm btn-outline-secondary me-1"
									onclick="location.href='${pageContext.request.contextPath}/schedule/myclubs?year=${prevYear}&month=${prevMonth}${searchParam}'">&lt;</button>
	
								<%-- ➡️ 다음 달 버튼 수정 --%>
								<button class="btn btn-sm btn-outline-secondary"
									onclick="location.href='${pageContext.request.contextPath}/schedule/myclubs?year=${nextYear}&month=${nextMonth}${searchParam}'">&gt;</button>
	
								<%-- 📅 Today 버튼 수정 --%>
								<button class="btn btn-sm btn-primary ms-3 rounded-pill"
									onclick="location.href='${pageContext.request.contextPath}/schedule/myclubs?${searchParam}'">Today</button>
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<div class="modal-body form-like-body">
						...
						<!-- 🔔 안내 문구 -->
						<div id="modalInfoBox" class="mb-3"></div>

						<hr class="my-2">

						<!-- 🔘 버튼들 -->
						<div id="modalButtons" class="d-flex flex-wrap gap-2"></div>
		    ...
		    fetch(ctx + '/schedule/detail?eventNo=' + eventNo)
		        ...
		        .then(data => {
		           ...
		           function addButton(label, onClick, color = "primary"){
			        ...
			        // 1️⃣ 일정 종료 + 이미 하비로그 있음 → 확인 버튼
					if (data.hasHobbyLog && data.hobbyLogId) {
					    addButton("하비로그 확인", function () {
					        location.href = ctx + "/hobbylog/detail/" + data.hobbyLogId;
					    }, "primary");
					}
					
					// 2️⃣ 작성 가능 → 기록 버튼
					else if (data.canWriteHobbyLog) {
					    addButton("하비로그 기록하기", function () {
					        location.href = ctx + "/hobbylog/write?eventNo=" + eventNo;
					    }, "success");
					}
					
					// 3️⃣ 참여는 했지만 아직 조건 미충족 → 안내 문구
					else if (data.isJoined) {
						let info = document.createElement("div");
						info.className = "text-muted small mb-3";
						info.innerText = "📝 이 일정이 종료되면 하비로그를 기록할 수 있어요.";
